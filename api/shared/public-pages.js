...
function listingCard(row) {
  ...
  return `<a class="related-card" href="/imovel/${encodeURIComponent(row.id)}"><img src="${escapeHtml(photo)}" alt=""><span class="related-copy"><b>${escapeHtml(item.title)}</b><small>${item.bedrooms ? `${item.bedrooms} quartos · ` : ''}${item.areaM2 ? `${item.areaM2} m²` : ''}</small><strong>${money(item.price)}</strong></span></a>`;
}
...
export async function handlePublicPageHtml(req, res) {
  try {
    ...
    const canonical = `${origin}/imovel/${encodeURIComponent(id)}`;
    ...
    const related = record.related.map(listingCard).join('');
    ...
    const body = item ? `<main><a class="back" href="/">← Catálogo</a><section class="hero"><div class="gallery">${gallery}</div><div class="intro"><div class="eyebrow">${escapeHtml(item.purpose)} · ${escapeHtml(item.type)}</div><h1>${escapeHtml(item.title)}</h1><p class="place">${escapeHtml(item.neighborhood)}</p><div class="price">${money(item.price)}</div><div class="stats">${attributes}</div>${item.description ? `<p class="description">${escapeHtml(item.description)}</p>` : ''}${item.features.length ? `<div class="features">${item.features.map((f) => `<span>${escapeHtml(f)}</span>`).join('')}</div>` : ''}<a class="cta" href="${phone ? `https://wa.me/${phone}?text=${msg}` : `https://wa.me/?text=${encodeURIComponent(`${item.title} ${canonical}`)}`}" target="_blank" rel="noopener">Tenho interesse neste imóvel</a><small class="agent">${escapeHtml(item.brand)} · ${escapeHtml(item.agentName)} — ${escapeHtml(item.agentRole)}</small></div></section>${relatedSection}</main>` : `<main><a class="back" href="/">← Catálogo</a><section class="unavailable"><div class="eyebrow">RM IMÓVEIS</div><h1>Este imóvel não está mais disponível</h1><p>Veja outras opções que podem combinar com você.</p>${related ? `<div class="related-grid">${related}</div>` : ''}</section></main>`;
